...
def bodies():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.commit()
            flash("Body entry added successfully!", "success")
    ...
    daily_history = (
        ...
        .filter(extract('year', CompletedTable.date) == today.year, extract('month', CompletedTable.date) == today.month)
        .group_by(CompletedTable.date)
        .order_by(CompletedTable.date.desc())
        .all()
    )

    daily_history_formatted = [{"date": row.date, "count": row.count, "serial_numbers": row.serial_numbers} for row in daily_history]

    return render_template('bodies.html', workers=workers, issues=issues, current_time=current_time, completed_tables=completed_tables, daily_history=daily_history_formatted)
...
def pods():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.commit()
            flash("Pod entry added successfully!", "success")
        except IntegrityError:
            db.session.rollback()
            flash("Error: Serial number already exists.", "error")
    ...
    daily_history = (
        ...
        .filter(extract('year', CompletedPods.date) == today.year, extract('month', CompletedPods.date) == today.month)
    ...
    return render_template('pods.html', workers=workers, issues=issues, current_time=current_time, completed_tables=completed_pods, daily_history=daily_history_formatted)
...
def top_rails():
    ...
    daily_history = (
        ...
        .filter(extract('year', TopRail.date) == today.year, extract('month', TopRail.date) == today.month)
    ...
    return render_template('top_rails.html', workers=workers, issues=issues, current_time=current_time, completed_tables=completed_top_rails, daily_history=daily_history_formatted)
